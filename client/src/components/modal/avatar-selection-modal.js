...
const AvatarSelectionModal = ({open, connectedAccounts, onAvatarModalClose, onAvatarChoose}) => {
    const context = useContext(AppContext);
    if (connectedAccounts.error) {
        return <React.Fragment/>
    }
    return <PageModal id="avatarSelection" isOpen={open} onHide={onAvatarModalClose} title="Choose Avatar" applyButton={<React.Fragment/>}>
        <Row className="mt-3 justify-content-center">
            <Col xs={5} sm={3} className="cursor-click text-center" onClick={() => onAvatarChoose(getDefaultAvatar(context.user.data.username))}>
            ...
            <Col xs={5} sm={3} className="cursor-click text-center"
                 onClick={() => onAvatarChoose("https://www.gravatar.com/avatar/" + md5(context.user.data.email) + ".jpg?s=100")}>
