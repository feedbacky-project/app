...
public class SortFilterResolver {
  ...
  public static Sort resolveIdeaSorting(BoardChangelogService.SortType sortType) {
    switch(sortType) {
      case OLDEST:
        return Sort.by(Sort.Order.by("pinned"), Sort.Order.asc("creationDate"));
      case NEWEST:
      default:
        return Sort.by(Sort.Order.by("pinned"), Sort.Order.desc("creationDate"));
    }
  }
  ...
  public static Sort resolveIdeaSorting(IdeaService.SortType sortType) {
    switch(sortType) {
      ...
      case OLDEST:
        return Sort.by(Sort.Order.by("pinned"), Sort.Order.asc("creationDate"));
      case VOTERS_ASC:
        return Sort.by(Sort.Order.by("pinned"), Sort.Order.asc("votersAmount"));
      case VOTERS_DESC:
        return Sort.by(Sort.Order.by("pinned"), Sort.Order.desc("votersAmount"));
      case TRENDING:
      default:
        return Sort.by(Sort.Order.by("pinned"), Sort.Order.desc("trendScore"));
    }
  }
